...
def visualize_pose(frame, keypoints, color=COLORS_BGR['green'], confidence_threshold=0.3, draw_skeleton=True, keypoint_colors=None):
    ...
    if draw_skeleton:
        for (a, b) in LIMBS:
            if (
                keypoints[a, 2] > confidence_threshold
                and keypoints[b, 2] > confidence_threshold
                and np.isfinite(keypoints[a, 0])
                and np.isfinite(keypoints[a, 1])
                and np.isfinite(keypoints[b, 0])
                and np.isfinite(keypoints[b, 1])
            ):
